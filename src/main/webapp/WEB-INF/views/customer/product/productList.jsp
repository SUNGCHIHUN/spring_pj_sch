...
<div id="container">
			...
			<div class="product">
				...
				<c:forEach var="p_dto" items="${plist}">
					<ul>
						<li>
							<%-- 품절인 경우 클릭 불가능 --%>
							<c:if test="${p_dto.product_state == 'soldout'}">
								<div class="word">
									품절
								</div>
								<img src="${p_dto.product_img_name}" alt="상품 이미지" width="300px" height="300px;">
							</c:if>
							<c:if test="${p_dto.product_state != 'soldout'}">
								<a href="${path}/productDetail.do?product_no=${p_dto.product_no}&product_category=${p_dto.product_category}"><img src="${p_dto.product_img_name}" alt="상품 이미지" width="300px" height="300px;"></a>
							</c:if>
						</li>
		...
		<div id="page">
			...
			<c:if test="${paging.startPage > 10}">
				<a href="${path}/productList.do?product_category=${product_category}&pageNum=${paging.prev}">[이전]</a>
			</c:if>
			...
			<c:forEach var="num" begin="${paging.startPage}" end="${paging.endPage}">
				...
				<c:if test="${num != paging.currentPage}">
					<a href="${path}/productList.do?product_category=${product_category}&pageNum=${num}">${num}</a>
				</c:if>
			</c:forEach>
			
			<%-- 다음버튼 활성화 여부 --%>
			<c:if test="${paging.endPage < paging.pageCount}">
				<a href="${path}/productList.do?product_category=${product_category}&pageNum=${paging.next}">[다음]</a>
			</c:if>
		</div>
	</div>
